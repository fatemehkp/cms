  ...
  mutate(
    ...
    ICD_CODE_main = 
      case_when(str_detect(ICD_CODE, "I") ~ "cvd",
                ...
                str_detect(ICD_CODE, "F01|F03|G30|G31") ~ "demn",
                str_detect(ICD_CODE, "E10|E11") ~ "diabt",
                str_detect(ICD_CODE, "N18|N19") ~ "kidn", 
                TRUE ~ as.character(ICD_CODE)),
    ### ICD sub causes
    ICD_CODE_sub = 
      ### cvd
...
```{r}
dt.cms.icd.rc.decoded %>% 
  filter(ICD_CODE_main == "seps") %>% # 228828
  filter(RECORD_COND_1_main == "seps") %>% # 228087
  nrow()

dt.cms.icd.rc.decoded %>% 
  filter(RECORD_COND_1_main == "seps") %>% # 700280
  filter(ICD_CODE_main == "seps") %>% # 228087
  nrow()
